...
// ==========================================
// RELATÓRIOS COM CORREÇÃO DE FUSO BRASÍLIA
// ==========================================
app.get('/api/relatorio-ultimo', proteger, async (req, res) => {
    try {
        ...
        // SQL: Converte UTC para Brasília de forma explícita na busca e na exibição
        let sql = `
            SELECT *, 
            TO_CHAR(data_hora AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo', 'DD/MM HH24:MI') as hora 
            FROM historico 
            WHERE (
                (data_hora AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo')::date = (NOW() AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo')::date
...
app.get('/api/relatorio-periodo', proteger, async (req, res) => {
    try {
        ...
        let sql = `
            ...
            TO_CHAR(data_hora AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo', 'DD/MM HH24:MI') as hora 
            FROM historico 
            WHERE (data_hora AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo')::date >= $1 
            AND (data_hora AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo')::date <= $2
        `;
...
// --- VIATURAS (GRAVAÇÃO SEGURA DE RASTRO) ---
...
app.put('/api/viaturas/:id', async (req, res) => {
    try {
        ...
        const vCheck = await pool.query("SELECT prefixo, km, setor, km_revisao FROM viaturas WHERE id = $1", [req.params.id]);
        const v = vCheck.rows[0];

        const kmAntigo = v.km;
        const prefixoVtr = v.prefixo;
        const setorVtr = setor || v.setor;
        const revVtr = km_revisao || v.km_revisao;
        const statusFinal = definirStatus(km, revVtr, status);
        ...
        await pool.query('UPDATE viaturas SET km=$1, km_revisao=$2, status=$3, ultimo_usuario=$4, motivo=$5, setor=$6 WHERE id=$7', 
            [km, revVtr, statusFinal, ultimo_usuario, motFinal, setorVtr, req.params.id]);
...
app.listen(PORT, () => console.log(`GEOFROTA ON (Fuso Brasil): Porta ${PORT}`));