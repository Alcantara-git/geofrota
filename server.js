...
// --- RELATÓRIOS COM FILTRO DE SETOR ---
...
app.put('/api/viaturas/:id', async (req, res) => {
    try {
        ...
        await pool.query('UPDATE viaturas SET prefixo=$1, placa=$2, modelo=$3, km=$4, km_revisao=$5, status=$6, ultimo_usuario=$7, motivo=$8, setor=$9 WHERE id=$10', 
            [prefixo, placa, modelo, km, km_revisao, st, ultimo_usuario, motivoFinal, setorReal, req.params.id]);
        
        await pool.query('INSERT INTO historico (prefixo, km_anterior, km_novo, status, motivo, usuario, setor) VALUES ($1,$2,$3,$4,$5,$6,$7)', 
            [prefixo, kmAnterior, km, st, motivoFinal, ultimo_usuario, setorReal]);
